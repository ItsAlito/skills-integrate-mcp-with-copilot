...
# Simple user database (username: {password, role})
users = {
...
def signup_for_activity(activity_name: str, email: str, user=Depends(get_current_user)):
    """Sign up a student for an activity (admin/counsellor only)"""
...
def unregister_from_activity(activity_name: str, email: str, user=Depends(get_current_user)):
    """Unregister a student from an activity (admin/counsellor only)"""
    if user["role"] not in ["admin", "counsellor"]:
        raise HTTPException(status_code=403, detail="Insufficient permissions")
    if activity_name not in activities:
        raise HTTPException(status_code=404, detail="Activity not found")
